...
export function InvoiceLineItemsCard({ detail }: { detail: InvoiceDetail }) {
  return (
    <Card className="flex h-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-text">Delivery Charges</h3>
        <span className="text-xs text-text-muted">{detail.lineItems.length} Active Line Items</span>
      </div>
      ...
      <div className="flex flex-col gap-1.5 border-t border-border pt-3 text-sm">
        ...
        <div className="flex items-center justify-between border-t border-border pt-1.5 text-base font-semibold">
          ...
          <span className="text-primary">{detail.totalDue}</span>
        </div>
      </div>
    </Card>
  );
